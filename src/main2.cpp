...
int main() {
  ...
  // Fundamental geometry filtering of putative matches
  {
    ...
    const std::pair<double,double> ACRansacOut =
            ACRANSAC(kernel, vec_inliers, 1024, &F,
                                 Square(4.0), // Upper bound of authorized threshold
                                  true);

    const double & thresholdF = ACRansacOut.first;  // Maximal authorized error threshold
    ...
    if (vec_inliers.size() > KernelType::MINIMUM_SAMPLES *2.5) {
      ...
      //没有像往常那样，直接用计算出来的vec_inliers 对匹配进行剔除，而是按照下面的策略进行剔除，
      //用计算出来的模型和描述子间的距离来剔除
      //结果发现：
      //在程序中的参数配置下，计算出来的inliers 个数竟大于直接用AC-RANSAC 计算出来的inliers 个数
